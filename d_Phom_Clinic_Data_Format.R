...
#First adding IDs
for (i in 1:length(list.raw.phom)) {
  Patient.ID <- str_split(names(list.raw.phom[i]), "_", n = 2)[[1]][1]
  Scan.ID <- str_split(names(list.raw.phom[i]), "_", n = 2)[[1]][2]
  
  list.all.data[[i]] <- c(Patient.ID = Patient.ID, Scan.ID = Scan.ID)
}
...
test.0 <- featcounter.vector.cubical.act(phom.test.crr, min = -1024, max = 3071, res = 1000, feat = 0)
test.1 <- featcounter.vector.cubical.act(phom.test.crr, min = -1024, max = 3071, res = 1000, feat = 1)
test.2 <- featcounter.vector.cubical.act(phom.test.crr, min = -1024, max = 3071, res = 1000, feat = 2)
test.tot <- cubical.total.feat.counter(phom.test.crr, min = -1024, max = 3071, res = 1000)
...
topfeatcurv.melt <- melt(list.all.data[[167]][["feat_curves"]], id.vars = c("HU"), 
...
for (i in 1:length(list.all.data)) {
...
list.all.data <- list.all.data[-which(sapply(list.all.data, length) == 4)]
...
for (i in 1:length(list.all.data)) {
  ...
  list.dimtot.mom <- list(dimtot.raw.moms = dimtot.raw.moms, dimtot.cent.moms = dimtot.cent.moms, 
                          dimtot.stand.moms = dimtot.stand.moms, dimtot.standcent.moms = dimtot.standcent.moms)
...
dups <- which(duplicated(sapply(list.all.data, function (x) x[["Patient.ID"]])))
